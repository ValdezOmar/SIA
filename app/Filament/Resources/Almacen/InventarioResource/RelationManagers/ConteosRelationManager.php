    ...
    public function table(Table $table): Table
    {
        return $table->modifyQueryUsing(fn ($query) => $query->with(['contador', 'revisor', 'articulo.fabricante']))
            ...
            ->columns([
                ...
                Filter::make('pendientes')->label('Pendientes de conteo')->query(fn ($query) => $query->whereNull('cantidad_contada')),
                Filter::make('diferencias')->label('Con diferencias')->query(fn ($query) => $query->whereNotNull('cantidad_contada')->whereColumn('cantidad_contada', '!=', 'stock_sistema')),
                Filter::make('sin_revision')->label('Pendientes de revisión')->query(fn ($query) => $query->whereNotNull('cantidad_contada')->whereNull('revisado_at')),
            ])->headerActions([
                Action::make('escanear')->label('Escanear QR / barras')->icon('heroicon-o-qr-code')
                    ->visible(fn () => $this->puedeContar())
                    ->schema([BarcodeInput::make('codigo')->label('Código de producto, barras o serie')->required()->maxLength(1000)])
                ...
                Action::make('revisar')->label('Revisar')->icon('heroicon-o-check-badge')
                    ...
                    ->schema([
                        ...
                        Placeholder::make('stock_actual')->label('Stock físico actual del sistema')
                            ->content(fn ($record) => Existencia::where('articulo_id', $record->articulo_id)->where('almacen_id', $this->getOwnerRecord()->almacen_id)->sum('cantidad_disponible'))
            ...
            ])->toolbarActions([])->defaultSort('codigo')->paginated([25, 50, 100])->defaultPaginationPageOption(50)->poll('30s')
