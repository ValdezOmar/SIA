...
class EventoPendienteResource extends Resource
{
            ...
            ->actions([
                ...
                Action::make('derivar')
                    ...
                    ->action(function (array $data, Evento $record) {
                        // 1. Actualizar el registro actual (ticket saliendo de la bandeja actual)
                        $record->update([
                        ...
                        // 2. Crear un NUEVO registro para la bandeja del destinatario
                        Evento::create([
                            'hd_ticket_id' => $record->hd_ticket_id,
                            'remitente_id' => $record->encargado_id, // El actual encargado se convierte en remitente
                            'destinatario_id' => $data['destinatario_id'], // El nuevo técnico
                            'area_origen_id' => $record->area_destino_id, // El área actual se convierte en origen
                        ...
                        // 3. Actualizar el ticket principal si existe
                        if ($record->ticket) {
    ...
    public static function getNavigationBadge(): ?string
    {
        $count = Auth::user()->empleado?->id
            ? static::getModel()::where('estado', 'pendiente')
