<div class="p-4 bg-white dark:bg-gray-900 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 space-y-5">
    ...
    @php
        $items = [
            ...
            ['label' => '≤ 4 meses', 'valor' => $menos4, 'bg' => 'bg-orange-100', 'bar' => 'bg-orange-500'],
            ['label' => '4–8 meses', 'valor' => $entre4y8, 'bg' => 'bg-amber-100', 'bar' => 'bg-amber-500'],
            ['label' => '> 8 meses', 'valor' => $mas8, 'bg' => 'bg-green-100', 'bar' => 'bg-green-500'],
    ...
    @foreach ($items as $item)
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center space-x-3 w-3/4 justify-end">
                ...
                <div class="relative w-32 h-2.5 rounded-full overflow-hidden {{ $item['bg'] }}">
                    <div class="{{ $item['bar'] }} h-full rounded-full transition-all duration-700 ease-in-out"
                         style="width: {{ $total ? round($item['valor'] / $total * 100, 1) : 0 }}%">
                    </div>
                </div>
            </div>
        </div>
